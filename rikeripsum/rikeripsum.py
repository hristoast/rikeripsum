...
def generate_sentence(word_count=None):
    ...
    by William Riker, and may in fact consisting of multiple sentences.
    ...
    if potential_matches:
        ...
        if word_count == 1:
            raise ImpossibleSentenceError("Couldn't generate a sentence with the requested number of words.")
...
class ImpossibleSentenceError(Exception):
    """
    Called when the engine is unable to fulfill the request due to lack
...
def main():
    ...
    parser.add_argument('-c', '--count', dest='count', type=int, help='minimum number of words in the sentence')
