...
def main():
    ...
    for season_num in range(1, 7):  # check every script for all seasons
        ...
        for script_file in os.listdir(season_dir):
            ...
            for data in f.readlines():  # look for pattern indicating character name
                if "<p>" in data and "<br>" in data:
                    splited = data.split("<p>")[-1].split("<br>")[0]
                    splited = splited.split("'S")[0].split(" ")
                    if len(splited) > 5:  # long list of words probably isn't a name
                        continue
                    for name in splited:
                        if name.isupper(): # character names are uppercase
                            charname = name
                            record = True
                            dbuffer = ""
                            break
